...
/**
 * PORT NOTE: {@code ArmorMaterial} is a registry record in 1.21, so each Valoria material is registered into
 * {@code Registries.ARMOR_MATERIAL} (see {@link #register(IEventBus)}) and armor items receive
 * {@code ArmorRegistry.X.material()} (a {@code Holder<ArmorMaterial>}). The registry name is the plain builder name
 * ({@code valoria:cobalt} etc.), which is also the armor layer asset ({@code textures/models/armor/cobalt_layer_N.png}).
 * Effect data now takes {@code Holder<MobEffect>}s.
 */
public class ArmorRegistry extends AbstractArmorRegistry{
    public static final DeferredRegister<ArmorMaterial> ARMOR_MATERIALS = DeferredRegister.create(Registries.ARMOR_MATERIAL, Valoria.ID);
    ...
    public static final ArmorRegistry FALLEN_COLLECTOR = new ArmorRegistry.Builder("fallen_collector").addAttr(AttributeReg.SUMMON_DAMAGE, new AttributeData(0.1f, Operation.ADD_MULTIPLIED_BASE)).addAttr(AttributeReg.NECROMANCY_COUNT, new AttributeData(1, Operation.ADD_VALUE)).protection(25).mul(45).enchantValue(20).ingredient(() -> Ingredient.of(Items.LEATHER)).build();
    public static final ArmorRegistry MARSH = new ArmorRegistry.Builder("marsh").addAttr(AttributeReg.SUMMON_DAMAGE, new AttributeData(0.05f, Operation.ADD_MULTIPLIED_BASE)).addAttr(AttributeReg.NECROMANCY_COUNT, new AttributeData(1, Operation.ADD_VALUE)).protection(20).mul(58).enchantValue(12).ingredient(() -> Ingredient.of(ItemsRegistry.marshCloth.get())).build();
    ...
    public static final List<ArmorRegistry> ALL = List.of(FALLEN_COLLECTOR, MARSH, BLACK_GOLD, COBALT, SAMURAI, ETHEREAL, NATURE, DEPTH, INFERNAL, SPIDER, PYRATITE, CRIMTANE, VOID, PHANTASM);

    /** Registers every material into the ARMOR_MATERIAL registry; must run before the items that use them are built. */
    public static void register(IEventBus eventBus){
        for(ArmorRegistry material : ALL){
            material.register(ARMOR_MATERIALS);
        }
    ...
    public static class Builder extends AbstractArmorBuilder<ArmorRegistry>{
        ...
        public Builder setVoidDefence(float def) {
            addAttr(AttributeReg.NIHILITY_RESISTANCE, new AttributeData(def, Operation.ADD_VALUE));
            return this;
        }
