...
/**
 * PORT NOTE: the {@code AttachCapabilitiesEvent<Entity>} handler is gone: the four player capabilities are data
 * attachments created lazily from their default suppliers ({@link ValoriaAttachments}). Tick events moved to
 * {@code PlayerTickEvent.Post}/{@code ServerTickEvent.Post} (no phase field). {@code reviveCaps/invalidateCaps} no
 * longer exist; attachment copying on clone is done by NeoForge, the explicit copyFrom calls are kept for the sync packets.
 */
public class CapabilityEvents{
    ...
    public void onValoriaEnter(Player player){
        ...
        if(level instanceof ServerLevel s && player instanceof ServerPlayer sp){
            ...
            // PORT NOTE: ServerAdvancementManager#getAdvancement(ResourceLocation) -> #get(ResourceLocation) returning an AdvancementHolder.
