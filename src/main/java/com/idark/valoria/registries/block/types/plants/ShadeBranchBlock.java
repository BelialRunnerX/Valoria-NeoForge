...
public class ShadeBranchBlock extends GrowingPlantHeadBlock{
    public static final com.mojang.serialization.MapCodec<ShadeBranchBlock> CODEC = simpleCodec(ShadeBranchBlock::new); // PORT NOTE: BlockBehaviour#codec() is abstract in 1.21
    @Override protected com.mojang.serialization.MapCodec<? extends ShadeBranchBlock> codec(){ return CODEC; }
    ...
    public void randomTick(BlockState pState, ServerLevel pLevel, BlockPos pPos, RandomSource pRandom) {
        if (pState.getValue(AGE) < 3 && net.neoforged.neoforge.common.CommonHooks.canCropGrow(pLevel, pPos.relative(this.growthDirection), pLevel.getBlockState(pPos.relative(this.growthDirection)),pRandom.nextDouble() < this.growPerTickProbability)) {
