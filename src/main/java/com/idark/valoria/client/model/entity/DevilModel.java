...
public class DevilModel<T extends Devil> extends AbstractHierarchicalModel<T> implements ArmedModel{
    ...
    public static LayerDefinition createBodyLayer(){
        ...
        PartDefinition Left_wing = Body.addOrReplaceChild("left_wing", CubeListBuilder.create().texOffs(0, 32).addBox(-9.0F, -5.5F, 0.0F, 18.0F, 11.0F, 0.0F, new CubeDeformation(0.0F)), PartPose.offsetAndRotation(8.9657F, -1.5F, 2.7844F, 0.0F, -0.0873F, 0.0F));

        PartDefinition Right_wing = Body.addOrReplaceChild("right_wing", CubeListBuilder.create().texOffs(32, 0).addBox(-9.0F, -5.5F, 0.0F, 18.0F, 11.0F, 0.0F, new CubeDeformation(0.0F)), PartPose.offsetAndRotation(-8.8786F, -1.5F, 2.7806F, 0.0F, 0.0873F, 0.0F));
    ...
    public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch){
        ...
        this.animateWalk(DevilAnimation.WALK, pLimbSwing, pLimbSwingAmount, 4, 8);
    ...
    private void animateWalk(float pLimbSwing, float pLimbSwingAmount){
        ...
        this.leftWing.yRot -= (0.25F * Mth.cos(f1 + (float)Math.PI) * f);
        this.rightWing.yRot += (0.25F * Mth.cos(f1 + (float)Math.PI) * f);
        this.resetArmPoses();
    }
