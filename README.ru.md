...
Jar-файлы публикуются только на [странице Releases](https://github.com/BelialRunnerX/Valoria-NeoForge/releases) этого репозитория (текущий: `v1.21.1-1.0.4.2`, отмечен как предрелиз). Также нужны [релиз порта Tridot](https://github.com/BelialRunnerX/Tridot-NeoForge/releases), Curios и GeckoLib (версии ниже). Порт не размещён на CurseForge или Modrinth.

## Состояние

- **Рабочая сборка, но не полностью проверенная.** Клиент запускается, создаёт и загружает миры; выделенный сервер запускается с чистым логом. Вручную проверено на данный момент (2026-09-24): Кодекс, курио и индикатор Нихилити, измерение Valoria, призыв четырёх боссов (полоски боссов, музыка, катсцена Фиррона), GUI станций крафта, дробилка камня, способности оружия и Phantasm Bow, размещение структур через `/locate`, JEI/Jade. Автоматический внутриигровой тест (`tools/porttest`) дополнительно проверил путешествие через портал в обе стороны, выполнение рецепта печи для обжига, доступность зачарований, бонусы атрибутов Curios, урон Нихилити, четыре боя с боссами с лутом, появление сущностей и разблокировку кодекса в крипте. Что ещё не проверено — в [KNOWN_ISSUES.ru.md](KNOWN_ISSUES.ru.md). Считайте сборки бета-версией и ожидайте ошибок в непроверенных областях.
