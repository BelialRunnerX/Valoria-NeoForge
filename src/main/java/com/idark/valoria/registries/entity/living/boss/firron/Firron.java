...
public class Firron extends Monster implements Enemy, BossEntity, Allied, AttackSystemMob, GeoEntity{
    ...
    // PORT NOTE: AttributeModifier(String name, ...) generated a fresh random UUID per call, so each phase transition stacked a
    // new modifier. Ids are ResourceLocations now and duplicates are rejected, hence the per-transition counter in the id.
    ...
    public void tick(){
        ...
        if(this.level().isClientSide()){
            playCutscene(); // PORT NOTE: DistExecutor removed; this branch is already guarded by level().isClientSide()
        } else if (tickCount == 1) {
