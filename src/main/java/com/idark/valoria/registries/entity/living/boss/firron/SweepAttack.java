...
public class SweepAttack extends AttackInstance{
    ...
    public boolean canPerformAttack(LivingEntity target){
        return isWithinAttackRange(target, Firron.sweepAttackRange);
    }

    @Override
    public void performAttack(){
        LivingEntity target = mob.getTarget();
        if (target != null && target.isAlive() && isWithinAttackRange(target, Firron.sweepAttackRange)){
