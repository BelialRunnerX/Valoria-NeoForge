...
public class FleshCystBlock extends BaseEntityBlock implements SimpleWaterloggedBlock{
    public static final com.mojang.serialization.MapCodec<FleshCystBlock> CODEC = simpleCodec(FleshCystBlock::new); // PORT NOTE: BlockBehaviour#codec() is abstract in 1.21
    @Override protected com.mojang.serialization.MapCodec<? extends FleshCystBlock> codec(){ return CODEC; }
    ...
    private void spawnSentinels(ServerLevel world, BlockPos pos){
        ...
        if(sentinels.size() <= 8){
            ...
            if(sentinel != null){
                ...
                if(world.noCollision(sentinel.getType().getSpawnAABB(d0, d1, d2))){ // PORT NOTE: getAABB -> getSpawnAABB
    ...
    // PORT NOTE: IBlockExtension#getExpDrop signature changed in NeoForge 21 (level accessor, block entity, breaker, tool).
    @Override
