...
public class ValoriaUtils{
    ...
    /**
     * PORT NOTE: replaces {@code PotionUtils.getCustomEffects(CompoundTag)} (removed in 1.20.5). Reads the same
     * {@code CustomPotionEffects} list the projectiles have always written, via {@code MobEffectInstance.load}.
     */
    ...
    @SuppressWarnings("deprecation")
    // PORT NOTE: MobEffectInstance takes Holder<MobEffect> in 1.21, so these return Holders.
    ...
    // PORT NOTE: the generic root-NBT integer helpers (only ever used for the "ValoriaRot" key) became component
    // helpers on DataComponentsRegistry.ROT; legacy "ValoriaRot" values are read from minecraft:custom_data.
    ...
    public static float enchantmentAccuracy(ItemStack stack) {
        int i = EnchantmentsRegistry.getLevel(stack, EnchantmentsRegistry.ACCURACY); // PORT NOTE: enchantments are data-driven ResourceKeys
    ...
    public static List<ItemStack> getLootTableItems(LootTable table) {
        ...
        for (LootPool pool : pools) {
            List<LootPoolEntryContainer> entries = pool.entries; // PORT NOTE: List instead of array; LootItem#item is a Holder
    ...
    public static boolean isCurable(MobEffectInstance e) {
        var effect = e.getEffect().value(); // PORT NOTE: MobEffectInstance#getEffect is a Holder
