...
public class BloodVinesFeature extends Feature<NoneFeatureConfiguration>{
   ...
   public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> pContext) {
      ...
      if (!worldgenlevel.isEmptyBlock(blockpos)) {
         ...
         if (!blockstate.is(TagsRegistry.MEAT)) {
            return false;
         } else {
   ...
   private void placeRoof(LevelAccessor pLevel, RandomSource pRandom, BlockPos pPos) {
      pLevel.setBlock(pPos, BlockRegistry.meatBlock.get().defaultBlockState(), 2);
      ...
      for(int i = 0; i < 200; ++i) {
         ...
         if (pLevel.isEmptyBlock(blockpos$mutableblockpos)) {
            ...
            if (j == 1) {
               pLevel.setBlock(blockpos$mutableblockpos, BlockRegistry.meatBlock.get().defaultBlockState(), 2);
            }
         }
      }
      ...
      for(int i = 0; i <= pHeight; ++i) {
         if (pLevel.isEmptyBlock(pPos)) {
            if (i == pHeight || !pLevel.isEmptyBlock(pPos.below())) {
               pLevel.setBlock(pPos, BlockRegistry.bloodVine.get().defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Integer.valueOf(Mth.nextInt(pRandom, pMinAge, pMaxAge))), 2);
               break;
            }

            pLevel.setBlock(pPos, BlockRegistry.bloodVinePlant.get().defaultBlockState(), 2);
         }
