...
- **Рабочая сборка, но не полностью проверенная.** Клиент запускается, создаёт и загружает миры, базовая игра работает (перемещение, стартовый набор, кодекс); выделенный сервер запускается и генерирует мир с чистым логом. Боссы, измерение Valoria, станции крафта, зачарования и остальные игровые системы ещё не тестировались. Считайте сборки бета-версией и ожидайте ошибок в непроверенных областях.
