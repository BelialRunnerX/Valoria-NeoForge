...
public class ShadewoodSpider extends Monster{
    ...
    // PORT NOTE: mirrors NeoForge's Spider patch; the manual MobEffectEvent.Applicable post became CommonHooks#canMobEffectBeApplied.
    public boolean canBeAffected(MobEffectInstance pPotioneffect){
        if(pPotioneffect.is(MobEffects.POISON)){
            return net.neoforged.neoforge.common.CommonHooks.canMobEffectBeApplied(this, pPotioneffect);
        }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData){
        ...
        if(pSpawnData instanceof Spider.SpiderEffectsGroupData spider$spidereffectsgroupdata){
            Holder<MobEffect> mobeffect = spider$spidereffectsgroupdata.effect; // PORT NOTE: SpiderEffectsGroupData.effect is a Holder in 1.21
            if(mobeffect != null){
