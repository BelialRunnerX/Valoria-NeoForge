...
- **Добавлены** `tools/porttest/porttest.js` (серверный скрипт KubeJS) и конфигурация запуска `clientAuto` (`gradlew runClientAuto` сразу открывает мир `PortTest`). Скрипт выполняет 46 проверок командами и через игровой API и пишет строки `[PORTTEST] PASS/FAIL` — без мыши и клавиатуры. Последний прогон 42/46; 4 неудачи оказались артефактами теста и переведены в информационные. Подробности и подводные камни — в `tools/porttest/README.md` (на английском); список проверенного перенесён в [KNOWN_ISSUES.ru.md](KNOWN_ISSUES.ru.md). Код мода в этой записи не менялся.
