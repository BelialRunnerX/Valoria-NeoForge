...
- **Fixed** 217 `Failed to parse recipe … No key id in {"count","item"}` warnings that appeared once the KubeJS plugin loaded: KubeJS' stock `item_stack` component only reads the 1.21 `{"id"}` form, while Valoria's recipe JSON uses the 1.20.1 `{"item"}` form. The schemas now use `valoria:item_stack` (shares the serializers' lenient codec) and `valoria:counted_ingredient` for the heavy workbench's `{"ingredient","count"}` entries; scripts pass `{ingredient: 'mod:item', count: n}` there. Recipes parsed this way are editable by scripts (`replaceOutput` etc.) instead of falling back to opaque vanilla JSON.
- Harness: 16 new checks (stone crusher interaction and loot, Bleeding proc, `neoforge:strippables` data map, item-based codex unlock, max-nihility KILL and TELEPORT actions, jewelry table / keg / soul infuser recipe execution, all six KubeJS schemas registering recipes, and script-added recipes executing in the kiln, jewelry table, crusher, keg and elemental manipulator). Full run 57/58 (the Necromancer treasure bag failed to drop once in seven runs; intermittent, under investigation).
