...
public class LootTableSubprovider extends BlockLootSubProvider {
    ...
    // PORT NOTE: BlockLootSubProvider needs the registry lookup in 1.21 (enchantment holders for fortune bonuses).
    ...
    protected void generate() {
        ...
        // PORT NOTE: Enchantments.BLOCK_FORTUNE -> Enchantments.FORTUNE, resolved to a Holder through the registry lookup.
        var fortune = this.registries.lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(Enchantments.FORTUNE);
        ...
        for (DeferredHolder<Block, ? extends Block> entry : BlockRegistry.BLOCK.getEntries()){
            ...
            if(!this.blocks.contains(block)){
                if(block instanceof SlabBlock slab){
                    ...
                    if(pot.getPotted() != null && pot.getPotted() != Blocks.AIR){ // PORT NOTE: FlowerPotBlock#getContent -> getPotted
