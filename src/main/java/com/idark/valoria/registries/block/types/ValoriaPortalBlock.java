...
/**
 * PORT NOTE: dimension travel in 1.21 goes through the vanilla {@link Portal} interface: {@code entityInside} marks the
 * entity as inside this portal and vanilla calls {@link #getPortalDestination} (transition time 0 = instant, as before).
 * {@code Entity#canChangeDimensions()} became {@code canUsePortal(false)}; {@code changeDimension(level, ITeleporter)}
 * became the {@code DimensionTransition} built by {@link ValoriaTeleporter}.
 */
    ...
    private void handlePortal(Entity player, BlockPos pPos){
        if(player.isOnPortalCooldown()){
            ...
            if(portalDimension != null && !player.isPassenger()){
                // PORT NOTE: the teleport itself is performed by vanilla via getPortalDestination once the entity is marked as inside this portal.
