...
public class TripleSweepAttack extends AttackInstance{
    private final float speedModifier;
    public int ticksUntilNextPathRecalc;
    public double lastTargetX, lastTargetY, lastTargetZ;

    public TripleSweepAttack(PathfinderMob mob, float range, float speedMod, int attackDelay, int attackDuration, int cooldown){
    ...
    public ResourceLocation getId(){
        return Valoria.loc("firron_triple_sweep");
    }
    ...
    public boolean canPerformAttack(LivingEntity target){
        return isWithinAttackRange(target, Firron.tripleSweepAttackRange);
    }
    ...
    public void performAttack(){
        LivingEntity target = mob.getTarget();
        if (target != null && target.isAlive() && isWithinAttackRange(target, Firron.tripleSweepAttackRange)){
            if(mob instanceof GeoEntity geo) {
                geo.triggerAnim("AttackController", "triple_sweep");
            }
