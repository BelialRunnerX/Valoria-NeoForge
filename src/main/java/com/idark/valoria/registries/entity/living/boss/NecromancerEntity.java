...
public class NecromancerEntity extends AbstractNecromancer implements BossEntity{
    ...
    public boolean isAlliedTo(Entity pEntity){
        if(super.isAlliedTo(pEntity)){
            return true;
        }else if(pEntity instanceof LivingEntity && pEntity.getType().is(net.minecraft.tags.EntityTypeTags.UNDEAD)){ // PORT NOTE: MobType -> minecraft:undead entity type tag
            ...
            if(NecromancerEntity.this.hasTarget()){
                ...
                for(LivingEntity entity : entities){
                    ...
                    if(sqrt != 0.0D){
                        ...
                        // PORT NOTE: ProtectionEnchantment.getExplosionKnockbackAfterDampener is gone in 1.21; Blast Protection now feeds the EXPLOSION_KNOCKBACK_RESISTANCE attribute.
                        double powerAfterDamp = power * (1.0D - entity.getAttributeValue(net.minecraft.world.entity.ai.attributes.Attributes.EXPLOSION_KNOCKBACK_RESISTANCE));
                        dX *= powerAfterDamp;
                        dY *= powerAfterDamp;
                        dZ *= powerAfterDamp;
