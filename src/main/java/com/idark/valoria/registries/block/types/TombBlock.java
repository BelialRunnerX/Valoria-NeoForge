...
public class TombBlock extends HorizontalDirectionalBlock implements SimpleWaterloggedBlock{
    ...
    public static final com.mojang.serialization.MapCodec<TombBlock> CODEC = com.mojang.serialization.codecs.RecordCodecBuilder.mapCodec(i -> i.group(
        com.mojang.serialization.Codec.BOOL.fieldOf("grave").forGetter(b -> b.isGrave),
        propertiesCodec()
    ).apply(i, TombBlock::new));
    private final boolean isGrave;

    @Override
    protected com.mojang.serialization.MapCodec<? extends TombBlock> codec(){
        return CODEC;
    }
